...
export default function MasksPanel() {
  ...
  const createMaskLogic = (type: Mask, mode: SubMaskMode = SubMaskMode.Additive) => {
    ...
    const params = subMask.parameters as Record<string, number>;
    ...
    if (type === Mask.Linear || type === Mask.Radial || type === Mask.Color || type === Mask.Luminance) {
      ...
      (subMask.parameters as Record<string, number | boolean>).isInitialDraw = true;
      if (type === Mask.Linear || type === Mask.Radial) {
        (subMask.parameters as Record<string, number>).startX = -10000;
        (subMask.parameters as Record<string, number>).startY = -10000;
        (subMask.parameters as Record<string, number>).endX = -10000;
        (subMask.parameters as Record<string, number>).endY = -10000;
        (subMask.parameters as Record<string, number>).centerX = -10000;
        (subMask.parameters as Record<string, number>).centerY = -10000;
        (subMask.parameters as Record<string, number>).radiusX = 0;
        (subMask.parameters as Record<string, number>).radiusY = 0;
      } else {
    ...
    if (type === Mask.AiDepth) {
      if (!params) subMask.parameters = {};
      (subMask.parameters as Record<string, number>).minDepth = 20;
...
function SubMaskRow({
  ...
  const typedSubMask = subMask as SubMask;
  const { t } = useTranslation();
  const { attributes, listeners, setNodeRef, isDragging } = useDraggable({
    id: subMask.id,
  ...
  const MaskIcon = MASK_ICON_MAP[typedSubMask.type] || Circle;
  const { showContextMenu } = useContextMenu();
